...
- **Branch protection:** `main` is governed by the `main-protection` repository ruleset: requires a PR, requires the `check`, `Analyze (actions)`, and `Analyze (javascript-typescript)` (CodeQL) status checks, and blocks deletions and non-fast-forward pushes. `required_approving_review_count` is **0** by design — autonomous Claude Code routines (see the `claude-routines` repo) open and auto-merge PRs unattended. The human-review gate is **path-scoped**, not blanket: `require_code_owner_review` is on, so any PR touching a path in `.github/CODEOWNERS` (CI, lockfiles, security invariants, input validation, redirect posture, rate limiting, all analyzers, orchestrator, scoring, DB migrations) requires a code-owner approval before merge. This hybrid keeps routine PRs autonomous while forcing a human on the security-sensitive minority. The repo Admin role bypasses the ruleset (`bypass_actors`, mode `always`) — **the autonomous bot must run as a distinct non-admin identity, never the admin account**, or the ruleset is advisory for it and CODEOWNERS cannot be satisfied (no self-approval).
- **CODEOWNERS scope (decided in #300):** The gate covers `src/analyzers/**`, `src/orchestrator.ts`, and `src/shared/scoring.ts` in addition to the CI/supply-chain and input-validation paths. Rationale: a PR adding a new exfiltrating module under `src/analyzers/` or modifying orchestrator/scoring to leak scan data would auto-merge without this coverage. The original two-path scope (mta-sts.ts + security-txt.ts) was intentionally narrow to preserve autonomy; we broadened it because the autonomy cost is low (routine PRs rarely touch stable analyzer logic) while the attack surface is real. Do not silently narrow this back — file a `docs:` PR with explicit rationale if re-narrowing is considered.
