...
export function renderDomainDetailPage({
  ...
  const historySection =
    scanHistory.length > 0
      ...
      : `<p style="color:var(--clr-text-muted);font-size:0.875rem;padding:0.75rem 0">No scan history yet. Scan results will appear here after the first check.</p>`;
...
function renderSparkline(entries: HistoryScanEntry[]): string {
  if (entries.length === 0) {
    return `<p style="color:var(--clr-text-muted);font-size:0.875rem;padding:0.75rem 0">No scans yet. Check back after your first scan to see the trend.</p>`;
  }
  const width = 600;
  const height = 80;
  const pad = 8;
...
function renderDriftTable(entries: HistoryScanEntry[]): string {
  if (entries.length === 0) {
    return `<p style="color:var(--clr-text-muted);font-size:0.875rem;padding:0.75rem 0">No scans yet. Changes to your domain's protocols will appear here after the next scan.</p>`;
  }
...
export function renderApiKeysPage({
  ...
  let table: string;
  if (keys.length === 0) {
    table = `<p style="color:var(--clr-text-muted);font-size:0.875rem">No API keys yet. Generate your first one above to start automating checks.</p>`;
  } else {
    const rows = keys
      .map((k) => {
